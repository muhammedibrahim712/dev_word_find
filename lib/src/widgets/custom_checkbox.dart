  ...
  Widget build(BuildContext context) {
    ...
    switch (effectiveMaterialTapTargetSize) {
      ...
      case MaterialTapTargetSize.shrinkWrap:
        size = const Size(
            kMinInteractiveDimension - 28.0, kMinInteractiveDimension - 28.0);
        break;
    }
...
class _CheckboxPainter extends ToggleablePainter {
  ...
  Paint _createOuterPaint() {
    return Paint()
      ..color = side?.color ?? checkColor
      ..style = PaintingStyle.stroke
      ..strokeWidth = strokeWidth;
  }
  ...
  void _drawBorder(Canvas canvas, Rect outer, double t, Paint paint) {
    ...
    if (side == null) {
      resolvedShape = resolvedShape.copyWith(
          side: BorderSide(width: strokeWidth, color: paint.color));
    }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Paint outerPaint = _createOuterPaint();
    ...
    if (previousValue == false || value == false) {
      ...
      if (t <= 0.5) {
        ...
        canvas.drawPath(emptyCheckboxPath, outerPaint);
      ...
      canvas.drawPath(
          shape.copyWith(side: side).getOuterPath(outer), outerPaint);
