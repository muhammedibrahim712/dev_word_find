  ...
  static Widget _builder(BuildContext context) {
    return Positioned.fill(
      child: GestureDetector(
        ...
        child: Stack(
          children: [
            Container(color: AppColors.commonBgForOverlay),
...
class OverlayProgressIndicator<B extends Bloc<dynamic, S>, S>
...
class _OverlayProgressIndicatorState<B extends Bloc<dynamic, S>, S>
    extends State<OverlayProgressIndicator> {
  ...
  bool _progressListenerWhen(S previous, S current) =>
      (previous is InProgressState &&
          current is InProgressState &&
          previous.inProgress != current.inProgress) ||
      (previous is! InProgressState && current is InProgressState) ||
      (previous is InProgressState && current is! InProgressState);

  /// Listener to obtain state from BLoC.
  void _progressListener(BuildContext context, S state) {
    if (state is InProgressState && state.inProgress) {
      _remove();
